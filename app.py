...
with tab4:
    ...
    if 'cost_sales' in tms_data and not tms_data['cost_sales'].empty:
        ...
        # === ROW 4: COST STRUCTURE PER SHIPMENT ===
        st.markdown("**Cost Breakdown per Shipment (Sample)**")
        if {'PU_Cost', 'Ship_Cost', 'Man_Cost', 'Del_Cost'}.issubset(cost_df.columns):
            sample_costs = cost_df[['PU_Cost', 'Ship_Cost', 'Man_Cost', 'Del_Cost']].head(50)
            sample_costs.index = [f"Shipment {i+1}" for i in range(len(sample_costs))]
            fig = px.bar(
                sample_costs,
                x=sample_costs.index,
                y=['PU_Cost', 'Ship_Cost', 'Man_Cost', 'Del_Cost'],
